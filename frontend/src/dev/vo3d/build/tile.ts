...
import { TILE, TILE_PHASE, tileMat, type MatKey } from "../render/Materials";
...
export function tiledFloor(rect: Rect, thickness = 1.2, tone?: MatKey, surface?: { roomId: string; label: string }): THREE.Mesh {
  const m = tileMat(tone);
  if (m.map) {
  ...
  // ROOM EDITOR: the one place every tiled floor in the office passes through, so tagging here makes each
  // room's floor addressable without a single room builder learning what an editor is.
  if (surface)
    tagSurface(mesh, { id: `${surface.roomId}/floor`, kind: "floor", roomId: surface.roomId, label: surface.label, preset: "tile", size: { u: rect.w, v: rect.d } });
  return mesh;
}
