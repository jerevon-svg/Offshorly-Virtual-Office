...
function ringShape(cx: number, cz: number, rIn: number, rOut: number, a0: number, a1: number): THREE.Shape {
...
function flatRing(shape: THREE.Shape, t: number, m: THREE.Material, y0: number): THREE.Mesh {
  const geo = new THREE.ExtrudeGeometry(shape, { depth: t, bevelEnabled: false, curveSegments: 64 });
...
function arcWall(r: number, rTop: number, h: number, y0: number, m: THREE.Material, a0: number, a1: number, cx: number, cz: number): THREE.Mesh {
  const geo = new THREE.CylinderGeometry(rTop, r, h, 96, 1, true, Math.PI / 2 - a1, a1 - a0);
