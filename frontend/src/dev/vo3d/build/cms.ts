...
function skirting(axis: "x" | "z", from: number, to: number, at: number): THREE.Mesh {
  return axis === "x"
    ? rbox(to - from - 1, 1.8, 1.0, mat(THEME.oakDark, 0.7), (from + to) / 2, 0, at, 0.2)
    : rbox(1.0, 1.8, to - from - 1, mat(THEME.oakDark, 0.7), at, 0, (from + to) / 2, 0.2);
}
...
export function cmsStatic(room: RoomDef, _opts: unknown): THREE.Group {
  ...
  g.add(skirting("x", WEST_X, EAST_X, SOUTH_WALL.z0 - 0.5));
  g.add(skirting("z", NORTH_Z, SOUTH_WALL.z0, EAST_X - 0.5));
  g.add(library());
  g.add(contentWall());
  g.add(teaPoint());
  g.add(eastWall());
