...
describe("live3dCharacters registry (bon promoted to bon-v3 2026-08-30, alex added 2026-08-29)", () => {
  it("bon resolves to the bon-v3 LODs per tier: T2 lod0, T1 lod1, static-frame bucket lod2", () => {
    ...
    expect(resolveLive3dGlbUrl(bon, "T2", false)).toMatch(/\/avatars\/bon-v3\/bon-v3-lod0\.glb$/);
    expect(resolveLive3dGlbUrl(bon, "T1", false)).toMatch(/\/avatars\/bon-v3\/bon-v3-lod1\.glb$/);
    expect(resolveLive3dGlbUrl(bon, "T0", true)).toMatch(/\/avatars\/bon-v3\/bon-v3-lod2\.glb$/);
    ...
    for (const url of Object.values(bon).filter((v): v is string => typeof v === "string")) {
      expect(url).toMatch(/\/avatars\/bon-v3\//);
    }
  });
