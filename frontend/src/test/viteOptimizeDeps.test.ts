...
// Regression guard for the Whiteboard create flow (2026-09-05): tldraw is loaded lazily, so
...
describe("vite.config.ts", () => {
  it("pre-bundles tldraw so opening a whiteboard never triggers a dev-server reload", () => {
    ...
    expect(optimizeDeps![1]).toMatch(/["']tldraw["']/);
  });
});
