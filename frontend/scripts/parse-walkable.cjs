 ...
 * (`src/assets/office/walkable-source.png`) into a precise walkability grid.
 *
 ...
 *
 * Renders a visual-verification overlay into scripts/_verify/ (gitignored):
...
const SCRATCH_DIR = path.join(__dirname, "_verify");

const REFERENCE_PNG = path.join(APP_ROOT, "src", "assets", "office", "walkable-source.png");
...
function writeOfficeWalkabilityGridTs(rows) {
  ...
  lines.push(`// image (src/assets/office/walkable-source.png), a ${COLS}x${ROWS} grid at ${CELL}px/tile.`);
  lines.push("");
