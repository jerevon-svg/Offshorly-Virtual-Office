#!/usr/bin/env node
/**
 * E2-v2 experiment (Track 1, avatar-generation feature): follow-up to E2
 * (generate-pose-e2.mjs). E2 showed camera-angle drift (came out near
 * eye-level/3-4-front instead of the correct 50-60deg high-overhead look)
 * even though identity was preserved. Fix under test: give the model a
 * VISUAL exemplar of the correct camera angle (a real production walk
 * frame), not just a text description.
 *
 ...
 *
 * Generates 4 candidates (4 separate calls - gpt-image-1 edits endpoint
 * does not reliably return multiple edit variations via n>1 for this
 * experiment's purposes, so we call 4x for independent samples).
 *
 * Standalone script. Not part of the Vite build. Run manually with:
 *   node scripts/avatar-pipeline/generate-pose-e2v2.mjs
 *
...
async function generateOne(apiKey, portraitBlob, exemplarBlob, index) {
  ...
  console.log(
    `E2-v2: calling OpenAI images/edits (gpt-image-1) candidate ${index}/${NUM_CANDIDATES} with 2 reference images...`
  );
...
async function main() {
  ...
  for (let i = 1; i <= NUM_CANDIDATES; i++) {
    ...
    const outPath = path.join(OUTPUT_DIR, `bon-walk-left-e2v2-${i}.png`);
    writeFileSync(outPath, buf);
    console.log(`E2-v2: wrote candidate ${i} to ${outPath}`);
    results.push(outPath);
  }

  console.log(`E2-v2: done. Generated ${results.length} candidates in ${OUTPUT_DIR}`);
}

main().catch((err) => {
  console.error(`E2-v2 FAILED: ${err?.message ?? err}`);
  process.exit(1);
});
