#!/usr/bin/env python3
"""Generates an animated (SMIL) SVG of the Import Candidates upload loader,
matching Figma node 14034:6314. No JS -- animates even when loaded via <img>."""
import math
...
STEPS = [
    "File uploaded",
    "Reading file and validating data",
    "Analyzing and preparing preview",
...
TOTAL = RAMP + HOLD
T_B = [0.675, 2.475, 3.825, RAMP]  # active_end time per step
...
<g filter="url(#cardShadow)">
  ...
  <g clip-path="url(#cardClip)">
    <text x="24" y="40" font-family="Source Serif 4, Georgia, serif" font-size="21" font-weight="500"
      fill="{DARK}" letter-spacing="-0.4">Import Candidates</text>

    <text x="24" y="76" font-family="Instrument Sans, Helvetica Neue, Arial, sans-serif" font-size="16"
      font-weight="500" fill="{DARK}">Uploading your file...</text>
    <text x="24" y="98" font-family="Instrument Sans, Helvetica Neue, Arial, sans-serif" font-size="14"
      fill="{GRAY}">Please don&#8217;t close this window. This may take a few moments.</text>
    ...
    <text x="{RING_CX}" y="270" text-anchor="middle" font-family="Instrument Sans, Helvetica Neue, Arial, sans-serif"
      font-size="14" font-weight="500" fill="{DARK}">candidate_import_new.csv</text>
    <text x="{RING_CX}" y="290" text-anchor="middle" font-family="Instrument Sans, Helvetica Neue, Arial, sans-serif"
      font-size="14" fill="{GRAY}">4.8 MB</text>

    <line x1="24" y1="319.5" x2="{W-24}" y2="319.5" stroke="{LINE}"/>

    {steps_svg}

    <line x1="0" y1="{H_TOP}" x2="{W}" y2="{H_TOP}" stroke="{LINE}"/>
    <circle cx="33" cy="{H_TOP+34}" r="9" fill="none" stroke="{GRAY}" stroke-width="1.4"/>
    <text x="33" y="{H_TOP+39}" text-anchor="middle" font-family="Georgia, serif" font-size="12"
      fill="{GRAY}">?</text>
    <text x="52" y="{H_TOP+39}" font-family="Instrument Sans, Helvetica Neue, Arial, sans-serif" font-size="14"
      fill="{DARK}">Need help? Check our <tspan font-weight="600" fill="{MINT}">import guide</tspan></text>
  </g>
</g>
</svg>'''
    return svg
...
if __name__ == "__main__":
    out = build_svg()
    path = "/Users/lekoffshorly/Documents/AI Agents/figma-assets/upload-loader/upload_loader.svg"
